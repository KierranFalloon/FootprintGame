...
def main():
    ...
    def new_pokemon() -> None:
        ...
        for i in positions:
            pokemon_in_boxes.append(
                *[
                    index
                    for index, p, in enumerate(collision_boxes)
                    if p.collidepoint(i)
                ]
            )
        ...
        pokemon_list = [
            ...
            pokemon_3,
            pokemon_4,
        ]  # List of pokemon generated, in order
    ...
    def update_screen() -> None:
        ...
        name_1_rect = name1.get_rect(
            center=tuple_subtraction(pokemon_1.position, (-50, -175))
        )
        screen.blit(name1, name_1_rect)

        if pokemon_1.pressed is True:
            footprint_rect = pokemon_1.footprint.get_rect(
                center=tuple_subtraction(pokemon_1.position, (-250, -100))
            )
            screen.blit(pokemon_1.footprint, footprint_rect)

        screen.blit(pokemon_2.sprite, pokemon_2.position)
        name_2_rect = name2.get_rect(
            center=tuple_subtraction(pokemon_2.position, (-50, -175))
        )
        screen.blit(name2, name_2_rect)

        if pokemon_2.pressed is True:
            footprint_rect = pokemon_2.footprint.get_rect(
                center=tuple_subtraction(pokemon_2.position, (-250, -100))
            )
            screen.blit(pokemon_2.footprint, footprint_rect)

        screen.blit(pokemon_3.sprite, pokemon_3.position)
        name_3_rect = name3.get_rect(
            center=tuple_subtraction(pokemon_3.position, (-50, -175))
        )
        screen.blit(name3, name_3_rect)

        if pokemon_3.pressed is True:
            footprint_rect = pokemon_3.footprint.get_rect(
                center=tuple_subtraction(pokemon_3.position, (-250, -100))
            )
            screen.blit(pokemon_3.footprint, footprint_rect)

        screen.blit(pokemon_4.sprite, pokemon_4.position)
        name_4_rect = name4.get_rect(
            center=tuple_subtraction(pokemon_4.position, (-50, -175))
        )
        screen.blit(name4, name_4_rect)
        if pokemon_4.pressed is True:
            footprint_rect = pokemon_4.footprint.get_rect(
                center=tuple_subtraction(pokemon_4.position, (-250, -100))
            )
    ...
    def change_text(label) -> None:
        ...
        for index, line in enumerate(label):
            screen.blit(
                label[index],
                (position[0], position[1] + (index * fontsize) + (15 * index)),
            )
    ...
    ratio = (
        background.get_size()[0] / bg.get_size()[0],
        background.get_size()[1] / bg.get_size()[1],
    )
    BG_center = (
        (SCREEN_WIDTH - (BG_WIDTH / 2) - 100),
        (SCREEN_HEIGHT - (BG_HEIGHT) - 50),
    )

    background_2 = pygame.transform.smoothscale(
        bg2, (SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2)
    )
    pokemon_boxes = [
        (0, 0),
        (SCREEN_WIDTH / 4, 0),
    ...
    text_box = pygame.image.load(
        os.path.join("Images", "text_box_blank.png")
    ).convert_alpha()
    text_box = pygame.transform.smoothscale(
        text_box, (SCREEN_WIDTH / 2, SCREEN_HEIGHT / 4)
    )

    # Timer screen utils
    red_timer = pygame.image.load(
        os.path.join("Images", "Red_timer.png")
    ).convert_alpha()
    red_timer = pygame.transform.smoothscale(
        red_timer,
        (red_timer.get_size()[0] * ratio[0], red_timer.get_size()[1] * ratio[1]),
    )
    green_timer = pygame.image.load(
        os.path.join("Images", "Green_timer.png")
    ).convert_alpha()
    green_timer = pygame.transform.smoothscale(
        green_timer,
        (green_timer.get_size()[0] * ratio[0], green_timer.get_size()[1] * ratio[1]),
    )
    timer_positions = [(1100 + i * 50, 675) for i in range(10)]

    # Pokemon box utils
    pokemon_box = pygame.image.load(os.path.join("Images", "Card.png")).convert_alpha()
    pokemon_box = pygame.transform.smoothscale(
        pokemon_box, (SCREEN_WIDTH / 4, SCREEN_HEIGHT / 4)
    )

    pokemon_box_highlight = pygame.image.load(
        os.path.join("Images", "Highlight.png")
    ).convert_alpha()
    pokemon_box_highlight = pygame.transform.smoothscale(
        pokemon_box_highlight, (SCREEN_WIDTH / 4, SCREEN_HEIGHT / 4)
    )

    # Collision detection
    pokemon_box_rect_1 = pokemon_box.get_rect(
        center=(SCREEN_WIDTH / 8, SCREEN_HEIGHT / 8)
    )
    pokemon_box_rect_2 = pokemon_box.get_rect(
        center=(3 * SCREEN_WIDTH / 8, SCREEN_HEIGHT / 8)
    )
    pokemon_box_rect_3 = pokemon_box.get_rect(
        center=(SCREEN_WIDTH / 8, 3 * SCREEN_HEIGHT / 8)
    )
    pokemon_box_rect_4 = pokemon_box.get_rect(
        center=(3 * SCREEN_WIDTH / 8, 3 * SCREEN_HEIGHT / 8)
    )
    ...
    while running:
        ...
        mouse_in_boxes = [
            index for index, p, in enumerate(collision_boxes) if p.collidepoint(mouse)
        ]
        ...
        if mouse_in_boxes == []:  # If not hovering a pokemon choice
            ...
            text_string = [
                "KF: Here comes a Pokémon! Check out it's footprint",
        ...
        if mouse_in_boxes != []:
            ...
            pokemon_hovering = pokemon_list[
                pokemon_in_boxes.index(mouse_in_boxes[0])
            ].name
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                clicked_sprites = [
                    index
                    for index, p, in enumerate(collision_boxes)
                    if p.collidepoint(mouse)
                ]  # See which box was clicked
                pokemon_clicked = pokemon_in_boxes.index(
                    *clicked_sprites
                )  # Reverse engineer which pokemon was clicked from box number

                text_string = [
                    "The footprint is {}'s!".format(
                        str(pokemon_list[pokemon_clicked].name)
                    ),
                    "The footprint is {}'s!".format(
                        str(pokemon_list[pokemon_clicked].name)
                    ),
                ...
                pokemon_list[
                    pokemon_clicked
                ].pressed = True  # Allow footprint to appear
        ...
        game_text_out_of_time_strings = [
            ...
            [
                "The correct answer is {}!".format(
                    *[i.name for i in pokemon_list if i.correct is True]
                ),
        ...
        try:
            stats_text = "P: {}, T: {}, {}".format(
                w, t, str(round(float(w) / float(t) * 100, 1))
            )
